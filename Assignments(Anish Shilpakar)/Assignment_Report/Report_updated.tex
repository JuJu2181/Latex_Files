...
Cross-Elasticity coefficient \cite{yang2019optimal} . A self-elasticity coefficient relates the load and
price at the same time while cross elasticity coefficients relate the load and price at
different times \cite{safdarian2014medium}. To capture the elastic behavior of load in response to the time
varying prices, a price elasticity matrix with self-elasticity and cross-elasticity coefficients equal to -0.2 and 0.0087 is utilized, respectively \cite{kirschen2003demand} .
\begin{figure}[H]
\centering
...
lowest prices, which typically are in the early hours of the morning \cite{kirschen2000factoring}.
\pagebreak
\section{Methodology}
...
Hourly load curve of a day can be divided into peak flat and valley period \cite{cao2011optimized}
.Equal step length iteration algorithm is used to optimize the period partitioning
\cite{yang2018reliability} is used in our project. Optimized period partition is divided into two parts.
